...
export function InternetIdentityProvider({
  ...
  // authClient stored in ref to avoid re-render loops
  const authClientRef = useRef<AuthClient | undefined>(undefined);
  // Track if init has run to avoid double-init in StrictMode
  ...
  // Initialize once on mount -- no state dependencies to avoid loops
  useEffect(() => {
