...
export function InternetIdentityProvider({
  ...
  // Keep createOptions stable via ref so the mount effect doesn't re-run
  ...
  }, [authClient, handleLoginError, setErrorMessage]);
  ...
  // Run only once on mount (and after logout which resets initializedRef)
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (!cancelled) {
        // Restore session = success, fresh visit = idle
        setStatus(wasAuthenticated ? "success" : "idle");
      }
    })();
    return () => {
      cancelled = true;
